...
void initCNN(){
    ...
    for (int i = 0; i < numConvolutionalLayers; i++){
        ...
        for (int j = 0; j < numFiltersAtConvolutionalLayers[i]; j++){ 
            ...
            randInitArray(filters[i][j], filterSizes[i]*filterSizes[i], -0.4f, 0.4f);
        ...
        randInitArray(filterBiases[i], numFiltersAtConvolutionalLayers[i], -1.0f, 1.0f);
    }
    ...
    for (int i = 0; i < numFullyConnectedLayers; i++){
        ...
        for (int j = 0; j < numNodesAtFullyConnectedLayers[i]; j++){
            ...
            randInitArray(fullyConnectedWeights[i][j], sizePrevLayer, -0.8, 1.0);
        }
        ...
        randInitArray(fullyConnectedBiases[i], numNodesAtFullyConnectedLayers[i], -1.0, 1.0);
    }
}
void destructCNN(){
    for (int i = 0; i < numConvolutionalLayers; i++){
...
int classifySample(image* Img, int startingSize){
    ...
    for (int i = 0; i < numNodesAtFullyConnectedLayers[numFullyConnectedLayers-1]; i++) {
        double expVal = exp(fullyConnectedZ[numFullyConnectedLayers-1][i])/temperature;
        // printf("Exp expVal: %f\n", expVal);
        // printf("Exp Sum: %f\n", expSum);
        if (isnan(expSum)){
            exit(0);
    ...
    int index;
    for (int i = 0; i< numNodesAtFullyConnectedLayers[numFullyConnectedLayers-1]; i++){
        fullyConnectedActivated[numFullyConnectedLayers-1][i] = (exp(fullyConnectedZ[numFullyConnectedLayers-1][i])/temperature)/expSum;
...
void backPropogate(image* Img, double learningRate){
    ...
    for (int i = 0; i < numNodesAtFullyConnectedLayers[numFullyConnectedLayers-1]; i++){
        ...
        fullyConnectedGradients[numFullyConnectedLayers-1][i] = labelledOutput[i] - fullyConnectedActivated[numFullyConnectedLayers-1][i];
    }
...
int main(){
    ...
    int numFiltersAtConvolutionalLayersArr[4] = {1, 32, 32, 2};
